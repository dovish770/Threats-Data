import axios, { SERVER_API } from "../service/axiosConfig";
...
export const fetchMostDeadliestService = async (): Promise<MostDeadliest[]> => {
  try {  
    const response: any = await axios.get("/api/analysis/deadliest-attack-types");    
...
export const fetchHighestAvgCasualtiesService = async (region: string, isAllRegion: string): Promise<HighestCasualties[]> => {
  try {
    const response = await fetch(SERVER_API + `api/analysis/highest-casualty-regions?region=${region}&isAllRegion=${isAllRegion}`);
...
export const fetchIncidentsTrendsService = async (startYear: string, endYear: string): Promise<incidentsTrends[]> => {
  try {
    const response = await fetch(SERVER_API +`api/analysis/incident-trends?startYear=${startYear}&endYear=${endYear}`);
...
export const fetchGangsByRegionService = async (region: string, top: string): Promise<gangsByRegion[]> => {
  try {
    const response = await fetch(SERVER_API + `api/relationships/top-groups?region=${encodeURIComponent(region)}&top=${top}`);
...
export const fetchListOfTypeService = async (type: string): Promise<string[]> => {
  try {
    const response = await fetch(SERVER_API + `api/list-of-types/?type=${type}`);
...
export const fetchGroupByYearOrGangService = async (year?: string, gang?: string):  Promise<IncidentsByYearOrGang[]> => {
  ...
  try {
    let url = SERVER_API + 'api/relationships/groups-by-year?';
    if (year) {
      url += `year=${year}`;
    } else if (gang) {
      url += `gang=${gang}`;
